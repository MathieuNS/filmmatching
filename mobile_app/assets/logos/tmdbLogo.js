/* ============================================================
   tmdbLogo.js — Logo officiel TMDB sous forme de chaîne SVG
   ------------------------------------------------------------
   Porté depuis `frontend/public/tmdb-logo.svg`. On le stocke en
   chaîne JS pour le dessiner avec `SvgXml` (react-native-svg),
   comme on le fait déjà pour les avatars.

   IMPORTANT : le SVG d'origine colorait le tracé via une CLASSE CSS
   (`.cls-1 { fill: url(#linear-gradient) }`) dans un bloc <style>.
   `SvgXml` ne sait PAS interpréter le CSS interne d'un SVG : on a
   donc retiré le <style> et appliqué le dégradé DIRECTEMENT sur le
   <path> avec `fill="url(#tmdb-gradient)"`. Sinon le logo s'afficherait
   en noir (fill par défaut).
   ============================================================ */

export const TMDB_LOGO_XML = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 185.04 133.4">
  <defs>
